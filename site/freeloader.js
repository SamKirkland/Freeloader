...
export function formatBitsPerSecond(bitsPerSecond) {
    ...
    if (bitsPerSecond >= 1e3)
        return `${(bitsPerSecond / 1e3).toFixed(0)} kbps`;
...
function weightedMean(samples) {
    ...
    for (const sample of samples) {
        if (!(sample.weight > 0) || !Number.isFinite(sample.value))
            continue;
...
function linearRegression(points) {
    ...
    for (const point of points) {
        ...
        if (!(weight > 0) || !Number.isFinite(point.x) || !Number.isFinite(point.y))
            continue;
        sw += weight;
        sx += weight * point.x;
        sy += weight * point.y;
...
function decayWeight(ageMs, halfLifeMilliseconds) {
    if (!(halfLifeMilliseconds > 0))
        return 1;
    if (ageMs <= 0)
        return 1;
 ...
 */
function mergeWindows(windows, gapMilliseconds = 0) {
    const valid = windows
    ...
    for (const window of valid) {
        if (current && window.start <= current.end + gapMilliseconds) {
            ...
            current.count += 1;
            if (window.fresh)
                current.freshBytes += window.bytes;
            current.members.push(window);
            busyMs += window.end - window.start;
        }
        else {
            if (current)
                current.concurrency = busyMs / Math.max(1, current.end - current.start);
            busyMs = window.end - window.start;
            current = {
                start: window.start,
                end: window.end,
...
function isThroughputSample(value) {
    if (!value || typeof value !== "object")
        return false;
    const sample = value;
    return (typeof sample["bytes"] === "number" &&
        ...
        sample["bitsPerSecond"] > 0);
}
function isLatencySample(value) {
    if (!value || typeof value !== "object")
        return false;
    const sample = value;
    return (typeof sample["timeToFirstByteMilliseconds"] === "number" &&
 ...
 * right now.
 */
function loadState(storage, key, maximumAgeMilliseconds, now) {
    ...
    if (state.version !== 1)
        return fresh;
    if (typeof state.updatedAt !== "number" || now - state.updatedAt > maximumAgeMilliseconds)
        return fresh;
    const cutoff = now - maximumAgeMilliseconds;
    const keepThroughput = (list) => Array.isArray(list) ? list.filter(isThroughputSample).filter((s) => s.at >= cutoff) : [];
...
function clearState(storage, key) {
    if (!storage)
        return;
    try {
 ...
 */
function parseTimingEntry(entry, options = {}) {
    ...
    const dns = entry.domainLookupEnd && entry.domainLookupStart && entry.domainLookupEnd > entry.domainLookupStart
        ? entry.domainLookupEnd - entry.domainLookupStart
        : null;
    const handshake = entry.connectEnd && entry.connectStart && entry.connectEnd > entry.connectStart
    ...
    const start = floor === null || floor === undefined || !Number.isFinite(floor)
        ? responseStart
...
class BurstAggregator {
    ...
    prune(horizon) {
        ...
        for (const burst of merged) {
            if (burst.end >= horizon)
                for (const window of burst.members)
                    keep.add(window);
        }
 ...
 */
function installUploadInstrumentation(host, options) {
    const now = options.now ?? (() => performance.now());
    const epochNow = options.epochNow ?? (() => Date.now());
    const minimumBytes = options.minimumBytes ?? 4096;
    const undo = [];
    let active = true;
    const emit = (bytes, durationMilliseconds, source) => {
        if (bytes < minimumBytes || !(durationMilliseconds > 1))
            return;
    ...
    if (typeof host.XMLHttpRequest === "function") {
        ...
        undo.push(() => {
            if (Original.prototype.send === patchedSend)
    ...
    if (typeof host.fetch === "function") {
        ...
        host.fetch = function patchedFetch(input, init) {
            ...
            const started = now();
            return originalFetch.call(host, input, init).then((response) => {
 ...
 */
function correctSlowStart(bytes, durationMilliseconds, concurrency, roundTripMilliseconds, strength = 1) {
    if (!roundTripMilliseconds || !(roundTripMilliseconds > 0) || strength <= 0 || !(durationMilliseconds > 0))
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (windowBytes <= initialWindow)
            return durationMilliseconds; // Never left the initial window.
...
const EMPTY_CONFIDENCE = { score: 0, samples: 0, bytes: 0, spread: 1 };
 ...
 */
function sizeWeight(bytes, minimumSampleBytes) {
    return Math.log2(1 + bytes / Math.max(1, minimumSampleBytes));
}
function scoreConfidence(samples, values, gatePenalty) {
    if (samples.length === 0)
    ...
    const score = clamp((sampleScore * 0.35 + byteScore * 0.4 + agreement * 0.25) * gatePenalty, 0, 1);
 ...
 */
function estimateDirection(allSamples, config, now) {
    ...
    for (const sample of usable) {
        ...
        if (!(weight > 0) || !(sample.bitsPerSecond > 0))
            continue;
        const durationMilliseconds = correctSlowStart(sample.bytes, sample.durationMilliseconds, sample.concurrency, config.roundTripMilliseconds, 
        ...
        if (bitsPerSecond > peak)
            peak = bitsPerSecond;
        if (sample.bytes >= largeBytes && bitsPerSecond > sustainedCeiling)
            sustainedCeiling = bitsPerSecond;
    }
    ...
    let regressionBitsPerSecond = null;
    ...
    if (sustainedCeiling > 0 && bitsPerSecond > sustainedCeiling)
        bitsPerSecond = sustainedCeiling;
 ...
 */
function trimmed(values, fraction = 0.1) {
    if (values.length < 5)
        return [...values];
    const sorted = [...values].sort((a, b) => a - b);
 ...
 */
function estimateLatency(samples, config, now) {
    ...
    const weighted = fresh.map((sample) => ({
...
export class Freeloader {
    ...
    start() {
        ...
        if (this.options.instrumentUploads) {
            this.uninstallUploads = installUploadInstrumentation(globalThis, {
     ...
     * previous page views.
     */
    firstByteFloorMilliseconds() {
        let floor = null;
        const consider = (value) => {
            if (Number.isFinite(value) && value >= 0 && (floor === null || value < floor))
                floor = value;
        };
        for (const sample of this.state.latency)
            consider(sample.timeToFirstByteMilliseconds);
        for (const pending of this.pendingLatency)
            consider(pending.sample.timeToFirstByteMilliseconds);
        return floor;
    }
    /**
    ...
    ingest(entry) {
        ...
        if (parsed.latency && parsed.wait) {
            ...
            if (this.pendingLatency.length > 256)
                this.pendingLatency.shift();
        }
        if (parsed.window) {
            ...
            if (this.recentWindows.length > 256)
                this.recentWindows.shift();
    ...
    flush(nowPerf) {
        ...
        if (live.length !== this.liveSamples.length || changed)
            changed = true;
        else
            changed = live.some((sample, i) => sample.bitsPerSecond !== this.liveSamples[i].bitsPerSecond);
    ...
    resolveLatency() {
        ...
        for (const item of pending) {
            ...
            const sample = { ...item.sample, contended };
    ...
    markContention(window) {
        for (const item of this.latencyWaits) {
            if (!item.sample.contended && this.overlaps(window, item))
                item.sample.contended = true;
        }
    }
    scheduleFlush() {
        if (this.flushTimer !== null)
    ...
    addSample(list, sample) {
        ...
        if (overflow > 0) {
            ...
            let best = departing[0];
            for (const candidate of departing)
                if (candidate.bitsPerSecond > best.bitsPerSecond)
                    best = candidate;
            list.unshift(best);
        }
    }
    ...
    estimatorConfig(roundTripMilliseconds) {
        return {
    ...
    recompute(now) {
        ...
        const downloadSamples = this.liveSamples.length > 0 ? [...this.state.download, ...this.liveSamples] : this.state.download;
    ...
    persist() {
        if (!this.options.persist)
            return;
        const download = this.liveSamples.length > 0
    ...
    timeOrigin() {
        if (this.options.timeOrigin !== undefined)
            return this.options.timeOrigin;
